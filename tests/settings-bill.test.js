describe("The factory function for both the bill with settings", function(){
    ...
    it("should update totals based on new costs if requested.", function() {
        ...
        phoneBill.setCallCost(2);
        phoneBill.setSMSCost(0.40);
        phoneBill.reCalculateTotal(true);

        assert.equal(phoneBill.getClass(),undefined);
        assert.equal(phoneBill.getCallCost(),2.00);
        assert.equal(phoneBill.getSMSCost(),0.40);
        assert.equal(phoneBill.getWarningLevel(),12.45);
        assert.equal(phoneBill.getCriticalLevel(),15.00);
        assert.equal(phoneBill.getCallCostTotal(),10.00);
        assert.equal(phoneBill.getCallCount(),5);
        assert.equal(phoneBill.getSMSCostTotal(),2.00);
        assert.equal(phoneBill.getSMSCount(),5)
        assert.equal(phoneBill.getTotalCost(),12.00);
